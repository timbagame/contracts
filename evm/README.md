...
## Client verification vectors

`test/fixtures/client-v1.json` records deterministic signing digests, game ID,
winner index and calldata generated by the real implementation through a proxy.
The fixture generator uses public local test identities and never broadcasts.

```bash
forge script script/ClientVectors.s.sol:ClientVectors
forge test --match-contract ClientVectorsTest
```

When intentionally changing the interface or encoding, review and update the
fixture from the generator's CLIENT_VECTOR output and synchronize client copies.
The test verifies the committed fixture on every EVM CI run.
