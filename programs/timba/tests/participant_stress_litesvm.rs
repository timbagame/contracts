...
use {
    ...
    timba::{
        state::{Game, GameType},
        utils::participant_hash,
        GameConfig,
    },
};

#[test]
fn fills_high_capacity_game_with_unique_ordered_hashes() {
    ...
    assert_eq!(state.participant_hashes.len(), 65);
    assert_eq!(state.total_amount, 48_750_000);
    for (index, (player, _)) in players.iter().enumerate() {
        assert_eq!(
            state.participant_hashes[index],
            participant_hash(&game, &player.pubkey())
        );
    }
}
