...
async function main() {
  ...
  const targetWallet = new PublicKey(
    "HhEWJstpJE6vvrYGS3BaK5ZJbdVAXqGmQ2MBM8FyiPvy"
  );

  // Single airdrop of 1000 SOL
  console.log(`Airdropping 1000 SOL to ${targetWallet.toString()}`);
  const success = await safeAirdrop(
    connection,
    targetWallet,
  ...
  if (authorityBalance < neededAmount) {
    ...
    console.log(
      `Authority wallet ${wallet.publicKey.toString()} already has sufficient funds.`
    );
  }
