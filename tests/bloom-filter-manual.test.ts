...
describe("Manual Bloom Filter Testing", () => {
  ...
  describe("Scenario 1: Filter Switching", () => {
    it("should demonstrate A/B filter switching when inactive filter expires", async () => {
      ...
      console.log(`Filter cleanup buffer: ${oracle.config.filterCleanupBuffer}s`);
      ...
      for (let i = 0; i < 3; i++) {
        ...
        await testUtils.game.initializeGame(gameData, shortConfig, player.player, mint.mint);
        await testUtils.game.joinGame(gameData.gamePDA, player.player);
        games.push(gameData);
        console.log(`   Created game ${i + 1}: ${gameData.gamePDA.toString().slice(0, 8)}...`);
      }

      // Step 2: Wait for filter switching opportunity
      const totalWaitTime = shortConfig.timeout + oracle.config.oracleBufferTime + oracle.config.filterCleanupBuffer + 2;
      console.log(`\\n2. Waiting ${totalWaitTime} seconds for filter switching...`);
      console.log("   (This simulates games expiring and becoming eligible for filter cleanup)");
      ...
      await testUtils.game.initializeGame(newGameData, longConfig, player.player, mint.mint);
      await testUtils.game.joinGame(newGameData.gamePDA, player.player); // This should trigger filter switch
      
      console.log(`   New game created: ${newGameData.gamePDA.toString().slice(0, 8)}...`);
  ...
  describe("Scenario 2: Filter Clearing", () => {
    it("should demonstrate filter clearing and reset behavior", async () => {
      ...
      for (let i = 0; i < 8; i++) {
        const gameData = testUtils.game.generateGamePDA();
        await testUtils.game.initializeGame(gameData, config, player.player, mint.mint);
        await testUtils.game.joinGame(gameData.gamePDA, player.player);
        recentGames.push(gameData);
        console.log(`   Game ${i + 1}/8: ${gameData.gamePDA.toString().slice(0, 8)}...`);
      }
      ...
      for (let i = 0; i < 2; i++) {
        ...
        console.log(`   Overflow game ${i + 1}/2: ${gameData.gamePDA.toString().slice(0, 8)}...`);
      }
  ...
  describe("Scenario 3: Long Unjoin Delays", () => {
    it("should handle stuck games and delayed unjoins without breaking filters", async () => {
      ...
      await testUtils.game.initializeGame(stuckGameData, stuckConfig, player1.player, mint.mint);
      ...
      console.log(`   Stuck game: ${stuckGameData.gamePDA.toString().slice(0, 8)}...`);
      console.log("   Both players joined, game is full but won't be completed by oracle");
      ...
      for (let i = 0; i < 12; i++) {
        const gameData = testUtils.game.generateGamePDA();
        await testUtils.game.initializeGame(gameData, normalConfig, player1.player, mint.mint);
      ...
      const unjoinWaitTime = stuckConfig.timeout + oracle.config.oracleBufferTime + 1;
      console.log(`\\n3. Waiting ${unjoinWaitTime} seconds for stuck game to become unjoin-able...`);
      console.log("   (Players are getting frustrated and want their money back!)");
      
      await new Promise((resolve) => setTimeout(resolve, unjoinWaitTime * 1000));
      ...
      const player1BalanceBefore = await env.program.account.playerBalance.fetch(player1.playerBalancePDA);
      const player2BalanceBefore = await env.program.account.playerBalance.fetch(player2.playerBalancePDA);
      
      console.log(`   Player1 balance before: ${player1BalanceBefore.amount.toString()}`);
      console.log(`   Player2 balance before: ${player2BalanceBefore.amount.toString()}`);
      ...
      // Verify refunds
      const player1BalanceAfter = await env.program.account.playerBalance.fetch(player1.playerBalancePDA);
      const player2BalanceAfter = await env.program.account.playerBalance.fetch(player2.playerBalancePDA);
      
      console.log(`   Player1 balance after: ${player1BalanceAfter.amount.toString()}`);
      console.log(`   Player2 balance after: ${player2BalanceAfter.amount.toString()}`);
      
      expect(player1BalanceAfter.amount.toNumber()).to.be.greaterThan(player1BalanceBefore.amount.toNumber());
      expect(player2BalanceAfter.amount.toNumber()).to.be.greaterThan(player2BalanceBefore.amount.toNumber());
      ...
      await testUtils.game.initializeGame(testGameData, normalConfig, player1.player, mint.mint);
      ...
      try {
        ...
        console.log("   ✅ Filter system still correctly preventing double joins");
      }

      console.log("   ✅ Filter system maintained integrity throughout the delayed unjoin scenario");
