...
describe("Merkle Tree Fuzz Tests", () => {
   ...
   */
  async function validateTreeIntegrity(
  ...
  describe("Basic Tree Operations Fuzz", () => {
    it("should maintain tree integrity under random join sequences", async function () {
      ...
      for (let iteration = 0; iteration < FUZZ_ITERATIONS; iteration++) {
        ...
        const players = await testUtils.player.createPlayerPool(
          numPlayers,
          mint.mint
        );
            ...
            if (numPlayers > 20 && i % 10 === 0) {
              console.log(
                `  Iteration ${iteration + 1}: ${
                  i + 1
                }/${numPlayers} players joined`
              );
            }
          } catch (error) {
            console.error(
              `Join failed at iteration ${iteration + 1}, player ${i + 1}:`,
              error
            );
            throw error;
          }
        ...
        if (iteration % 10 === 0) {
          console.log(
            `  Completed iteration ${
              iteration + 1
            }/${FUZZ_ITERATIONS} (${numPlayers} players)`
          );
        }
      }
    }).timeout(TIMEOUT_MS);
    ...
    it("should handle random join and roll sequences for snowball games", async function () {
      ...
      for (let iteration = 0; iteration < FUZZ_ITERATIONS / 2; iteration++) {
        // Fewer iterations for complex test
        const { mint } = await testUtils.quickSetup();

        // Random game configuration for snowball
        const numPlayers = randomInt(2, Math.min(MAX_PLAYERS / 2, 25)); // Smaller for performance
        const rollsPerPlayer = randomInt(
          0,
          Math.min(MAX_ROLLS_PER_PLAYER / 2, 10)
        ); // Smaller for performance
        ...
        for (let rollRound = 0; rollRound < rollsPerPlayer; rollRound++) {
          ...
          for (const player of shuffledPlayers) {
            try {
              ...
              const rollProof = generateMerkleProof(
                currentPlayerArray,
                ticketIndex,
                currentGameState
              );
              ...
              console.error(
                `Roll failed at iteration ${iteration + 1}, round ${
                  rollRound + 1
                }:`,
                error
              );
        ...
        if (iteration % 5 === 0) {
          console.log(
            `  Completed iteration ${iteration + 1}/${Math.floor(
              FUZZ_ITERATIONS / 2
            )} (${numPlayers} players, ${rollsPerPlayer} rolls each)`
          );
        }
      }
    }).timeout(TIMEOUT_MS);
  ...
  describe("Proof Generation Fuzz", () => {
    it("should generate valid proofs for all positions in random trees", async function () {
      ...
      for (let iteration = 0; iteration < FUZZ_ITERATIONS / 4; iteration++) {
        // Fewer iterations for intensive test
        const { mint } = await testUtils.quickSetup();
        ...
        for (let ticketIndex = 0; ticketIndex < numPlayers; ticketIndex++) {
          try {
            ...
            console.error(
              `Proof generation failed at iteration ${
                iteration + 1
              }, ticket ${ticketIndex}:`,
              error
            );
        ...
        if (iteration % 5 === 0) {
          console.log(
            `  Completed iteration ${iteration + 1}/${Math.floor(
              FUZZ_ITERATIONS / 4
            )} (${numPlayers} players, all proofs generated)`
          );
        }
      }
    }).timeout(TIMEOUT_MS);
    ...
    it("should generate consistent proofs across different tree configurations", async function () {
      ...
      for (let iteration = 0; iteration < FUZZ_ITERATIONS / 8; iteration++) {
        // Fewer iterations for very intensive test
        const { mint } = await testUtils.quickSetup();
        ...
        for (let rollCount = 0; rollCount < maxRolls; rollCount++) {
          ...
          const gameStateBefore = await env.program.account.game.fetch(
            gameData.gamePDA
          );
          const proofBefore = generateMerkleProof(
            players,
            ticketIndex,
            gameStateBefore
          );
          ...
          const gameStateAfter = await env.program.account.game.fetch(
            gameData.gamePDA
          );
        ...
        if (iteration % 2 === 0) {
          console.log(
            `  Completed iteration ${iteration + 1}/${Math.floor(
              FUZZ_ITERATIONS / 8
            )} (${initialPlayers} players, ${maxRolls} rolls)`
          );
        }
      }
    }).timeout(TIMEOUT_MS);
    ...
    it("should maintain structural integrity under random operations", async function () {
      ...
      for (let iteration = 0; iteration < FUZZ_ITERATIONS / 4; iteration++) {
        ...
        for (let i = 0; i < numPlayers; i++) {
          ...
          // Capture tree state
          const gameState = await env.program.account.game.fetch(
            gameData.gamePDA
          );
          treeStates.push({
  ...
  describe("Performance and Scalability Fuzz", () => {
    it("should maintain reasonable performance with larger trees", async function () {
      ...
      for (let iteration = 0; iteration < 10; iteration++) {
        // Fewer iterations for performance tests
        const { mint } = await testUtils.quickSetup();
        ...
        console.log(
          `  ${numPlayers} players: ${joinDuration}ms joins, ${proofDuration}ms proofs`
        );
      ...
      for (const result of performanceResults) {
        console.log(
          `  ${result.players} players: ${result.joinTimePerPlayer.toFixed(
            1
          )}ms/join, ${result.proofTimePerProof.toFixed(1)}ms/proof`
        );
      }
      ...
      const joinScaleFactor =
        lastResult.joinTimePerPlayer / firstResult.joinTimePerPlayer;
      ...
      console.log(
        `  Scale factors: joins ${joinScaleFactor.toFixed(
          2
        )}x, proofs ${proofScaleFactor.toFixed(2)}x`
      );
